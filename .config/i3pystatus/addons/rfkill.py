...
class Rfkill(File):
    settings = (
        ...
        ("rfkill_override_command_name",
         "It will execute `rfkill [un]block (this name)`"),
        "color",
        "down_color",
    )
    required = ()
    ...
    rfkill = "*"
    rfkill_name = None
    rfkill_override_command_name = None
    ...
    def rfkill_switch(self):
        run_through_shell(
            [
                ...
                str(self.data["index"]) if self.rfkill_override_command_name is None else self.rfkill_override_command_name,
            ]
        )
